...
class CFG:
    ...
    pretrained_path: str = '/home/yq/models/vit_large_patch16_dinov3_qkvb.lvd1689m/pytorch_model.bin'
    data_dir: Path = field(default_factory=lambda: Path(os.environ.get("CSIRO_DATA_DIR", "./input/csiro-biomass")))
    output_dir: Path = field(default_factory=lambda: Path(os.environ.get("OUTPUT_DIR", "output/exp-v147")))
...
def run_fold(cfg: CFG, df: pd.DataFrame, fold: int, device: torch.device) -> tuple[float, np.ndarray]:
    ...
    train_dataset = BiomassDataset(
        train_df, lambda: get_train_transforms(cfg.img_size), cfg.image_dir, cfg.target_train, cfg.target_eval
    )
