...
class CFG:
    ...
    output_dir: Path = field(default_factory=lambda: Path(os.environ.get("OUTPUT_DIR", "output/exp-v148")))
    resume_path: Optional[str] = 'output/exp-v148/fold0_best.pth'  # 微调权重路径，如果提供则加载并继续训练
    start_epoch: int = 0  # 恢复训练的起始 epoch
    augment_epochs: int = 15  # Number of epochs to use augmented views
    ...
    def __post_init__(self) -> None:
        ...
        # Height normalization stats (will be computed from train data)
        self.height_mean: float = 0.0
        self.height_std: float = 1.0
...
def load_train_dataframe(cfg: CFG) -> pd.DataFrame:
    ...
    LOGGER.info("State encoding: %d unique states found - %s", cfg.num_states, cfg.state_encoder.classes_.tolist())
    
    # 5. 对 Species 进行 Label Encoding
    # 处理缺失值，用空字符串填充
    ...
    # 6. 计算 Height_Ave_cm 的归一化统计量 (标准化到均值0-标准差1)
    cfg.height_mean = df_wide['Height_Ave_cm'].mean()
    cfg.height_std = df_wide['Height_Ave_cm'].std()
    df_wide['Height_normalized'] = (df_wide['Height_Ave_cm'] - cfg.height_mean) / cfg.height_std
    LOGGER.info("Height normalization: mean=%.4f, std=%.4f", cfg.height_mean, cfg.height_std)
...
def get_tta_transforms(img_size: int) -> list[A.Compose]:
    ...
    original_view = A.Compose([A.Resize(img_size, img_size), normalize, ToTensorV2()])
    hflip_view = A.Compose([A.Resize(img_size, img_size), A.HorizontalFlip(p=1.0), normalize, ToTensorV2()])
    vflip_view = A.Compose([A.Resize(img_size, img_size), A.VerticalFlip(p=1.0), normalize, ToTensorV2()])
    rotate90_view = A.Compose([A.Resize(img_size, img_size), A.Rotate(limit=(90, 90), p=1.0, border_mode=0), normalize, ToTensorV2()])
    
    return [hflip_view, vflip_view, rotate90_view, original_view]


def get_train_transforms_for_epoch(img_size: int, epoch: int, augment_epochs: int = 15) -> tuple[A.Compose, int]:
    """Get appropriate transform based on current epoch with random augmentation selection."""
    tta_transforms = get_tta_transforms(img_size)
    view_idx = random.randint(0, 3)
    return tta_transforms[view_idx], view_idx
...
class BiomassDataset(Dataset):
    ...
    def __init__(
        ...
        # Meta features: categorical (int) and numerical (float)
        self.state_encoded = self.df["State_encoded"].values.astype(np.int64)
        self.species_encoded = self.df["Species_encoded"].values.astype(np.int64)
        self.ndvi = self.df["Pre_GSHH_NDVI"].values.astype(np.float32)
        self.height_normalized = self.df["Height_normalized"].values.astype(np.float32)
    ...
    def set_transform(self, transform: A.Compose) -> None:
        ...
        self.transforms_fn = lambda: transform
    ...
    def __getitem__(self, idx: int):
        ...
        # Meta features
        state = torch.tensor(self.state_encoded[idx])
        species = torch.tensor(self.species_encoded[idx])
        # Numerical meta features stacked as [NDVI, Height_normalized]
        meta_num = torch.tensor([self.ndvi[idx], self.height_normalized[idx]], dtype=torch.float32)
        
        return left, right, full_image, train_tgt, eval_tgt, state, species, meta_num
...
class BiomassModel(nn.Module):
    """Shared backbone with regression heads, using meta features via embeddings."""

    def __init__(self, model_name: str, pretrained: bool = True, pretrained_path: str = '', 
                 num_states: int = 4, num_species: int = 15, 
                 state_embed_dim: int = 8, species_embed_dim: int = 16):
        super().__init__()
        if pretrained_path:
        ...
        # Image features from 3 streams
        self.n_features_img = self.backbone.num_features * 3
        
        # Embedding layers for categorical features (State, Species)
        ...
        # Numerical meta features: NDVI (already 0-1), Height (normalized)
        num_numerical_features = 2  # NDVI, Height_normalized
        
        # Total feature dimension = image features + state embed + species embed + numerical features
        self.n_features = self.n_features_img + state_embed_dim + species_embed_dim + num_numerical_features
        
        # Only 3 prediction heads for competition targets
    ...
    def forward(self, left: torch.Tensor, right: torch.Tensor, full_image: torch.Tensor,
                state: torch.Tensor, species: torch.Tensor, meta_num: torch.Tensor):
        # Extract image features from 3 streams
        ...
        state_emb = self.state_embedding(state)  # [B, state_embed_dim]
        species_emb = self.species_embedding(species)  # [B, species_embed_dim]
        
        # Concatenate all features: image + state embed + species embed + numerical
        feats = torch.cat([feats_img, state_emb, species_emb, meta_num], dim=1)
        
        # Each head returns (mean, var) tuple - only predict competition targets
...
class PhysicsConsistencyLoss(nn.Module):
    """
    Physics-consistent loss using GaussianNLLLoss (simplified, no auxiliary tasks).
    ...
    def __init__(self, weights: dict[str, float] = None):
        ...
        if weights is None:
            ...
            # 从配置中映射权重，如果没有则使用默认值
            self.w = {
                "total": weights.get("total_loss", 0.5),
        ...
        # self.criterion_aux = nn.SmoothL1Loss(beta=1.0) # Removed
        # Classification loss for species
        # self.classification_criterion = nn.CrossEntropyLoss() # Removed

    def forward(self, preds, targets):
        """
        preds: tuple of ((total_mean, total_var), (gdm_mean, gdm_var), (green_mean, green_var))
        targets: tensor of shape [Batch, 3] -> [Total, GDM, Green]
        """
        (pred_total_mean, pred_total_var), \
        (pred_gdm_mean, pred_gdm_var), \
        (pred_green_mean, pred_green_var) = preds
...
def train_one_epoch(
    ...
    for step, (left, right, full_image, targets, _, state, species, meta_num) in pbar:
        left = left.to(device)
        ...
        with autocast(device_type='cuda', enabled=cfg.apex):
            preds = model(left, right, full_image, state, species, meta_num)
...
def valid_one_epoch(
    ...
    """
    验证一个 epoch，返回损失、加权 R2 分数和每个目标的 R2 分数。
    
    Returns:
        tuple: (val_loss, score, r2_scores)
            - val_loss: 验证损失
    ...
    try:
        with torch.no_grad():
            ...
            for left, right, full_image, train_targets, eval_targets, state, species, meta_num in pbar:
                left = left.to(device)
                right = right.to(device)
                full_image = full_image.to(device)
                ...
                with autocast(device_type='cuda', enabled=cfg.apex):
                    # Model returns tuples of (mean, var) for each head: (total, gdm, green)
                    preds = model(left, right, full_image, state, species, meta_num)
                    loss = criterion(preds, train_targets)

                # Extract mean values for R2 calculation (3 heads now)
                (pred_total_mean, _), (pred_gdm_mean, _), (pred_green_mean, _) = preds
    ...
    return losses.avg, score, r2_scores
...
def run_fold(cfg: CFG, df: pd.DataFrame, fold: int, device: torch.device) -> tuple[float, np.ndarray]:
    ...
    # Initialize with epoch 1 transform (will be updated each epoch)
    initial_train_transform, _ = get_train_transforms_for_epoch(cfg.img_size, epoch=1, augment_epochs=cfg.augment_epochs)
    train_dataset = BiomassDataset(
        train_df, lambda: initial_train_transform, cfg.image_dir, cfg.target_train, cfg.target_eval
    )
    valid_dataset = BiomassDataset(
        valid_df, lambda: get_valid_transforms(cfg.img_size), cfg.image_dir, cfg.target_train, cfg.target_eval
    )
    ...
    if cfg.resume_path is not None:
        ...
        if resume_path.exists():
            ...
            LOGGER.info("Checkpoint loaded successfully, will start from epoch %d", cfg.start_epoch)
        else:
            LOGGER.warning("Checkpoint path %s does not exist, starting from scratch", resume_path)
    ...
    for epoch in range(1, cfg.freeze_epochs + 1):
        if epoch < cfg.start_epoch:
            continue
        # Update TTA-style transform for this epoch (randomly selected)
        current_transform, view_idx = get_train_transforms_for_epoch(cfg.img_size, epoch, cfg.augment_epochs)
        train_dataset.set_transform(current_transform)
        view_names = ["hflip", "vflip", "rotate90", "original"]
        LOGGER.info("Fold %d Epoch %d/%d (frozen) - randomly using %s view", fold, epoch, cfg.epochs, view_names[view_idx])
        ...
        val_loss, score, r2_scores = valid_one_epoch(cfg, valid_loader, model, criterion, device)
    ...
    for epoch in range(cfg.freeze_epochs + 1, stage2_end):
        ...
        train_dataset.set_transform(current_transform)
        view_names = ["hflip", "vflip", "rotate90", "original"]
        LOGGER.info("Fold %d Epoch %d/%d (finetune) - randomly using %s view", fold, epoch, cfg.epochs, view_names[view_idx])
        train_loss = train_one_epoch(cfg, train_loader, model, criterion, optimizer, scaler, device)
        LOGGER.info("Fold %d Epoch %d -> train_loss %.4f", fold, epoch, train_loss)
        val_loss, score, r2_scores = valid_one_epoch(cfg, valid_loader, model, criterion, device)
        step_scheduler(scheduler, val_loss)
    ...
    if cfg.ema_epochs < cfg.epochs:
        for epoch in range(cfg.ema_epochs + 1, cfg.epochs + 1):
            ...
            view_names = ["hflip", "vflip", "rotate90", "original"]
            if cfg.use_ema:
                LOGGER.info("Fold %d Epoch %d/%d (ema) - randomly using %s view", fold, epoch, cfg.epochs, view_names[view_idx])
            else:
                LOGGER.info("Fold %d Epoch %d/%d (finetune) - randomly using %s view", fold, epoch, cfg.epochs, view_names[view_idx])
            train_loss = train_one_epoch(cfg, train_loader, model, criterion, optimizer, scaler, device,ema)
            LOGGER.info("Fold %d Epoch %d -> train_loss %.4f", fold, epoch, train_loss)
            val_loss, score, r2_scores = valid_one_epoch(cfg, valid_loader, model, criterion, device, ema)
            step_scheduler(scheduler, val_loss)
            if score > best_score:
                best_score = score
                torch.save(model.state_dict(), best_path)
    ...
    _, final_score, final_r2_scores = valid_one_epoch(cfg, valid_loader, model, criterion, device)
