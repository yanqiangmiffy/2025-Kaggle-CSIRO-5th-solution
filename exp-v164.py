...
class CFG:
    ...
    output_dir: Path = field(default_factory=lambda: Path(os.environ.get("OUTPUT_DIR", "output/exp-v164")))
...
def get_train_transforms_for_epoch(img_size: int, epoch: int, augment_epochs: int = 15) -> A.Compose:
    """Get appropriate transform based on current epoch.
    ...
    Returns:
        A.Compose transform for the current epoch
    """
    tta_transforms = get_tta_transforms(img_size)
    
    if epoch <= augment_epochs:
        # First 15 epochs: cycle through augmented views (hflip, vflip, rotate90)
        # tta_transforms[0:3] are hflip, vflip, rotate90
        view_idx = (epoch - 1) % 3  # Cycle through indices 0, 1, 2
        return tta_transforms[view_idx]
    else:
        # Last 10 epochs: use only original view
        return tta_transforms[3]  # original_view
...
def run_fold(cfg: CFG, df: pd.DataFrame, fold: int, device: torch.device) -> tuple[float, np.ndarray]:
    ...
    initial_train_transform = get_train_transforms_for_epoch(cfg.img_size, epoch=1, augment_epochs=cfg.augment_epochs)
    ...
    for epoch in range(1, cfg.freeze_epochs + 1):
        # Update TTA-style transform for this epoch
        current_transform = get_train_transforms_for_epoch(cfg.img_size, epoch, cfg.augment_epochs)
        train_dataset.set_transform(current_transform)
        view_names = ["hflip", "vflip", "rotate90", "original"]
        view_idx = (epoch - 1) % 3 if epoch <= cfg.augment_epochs else 3
        LOGGER.info("Fold %d Epoch %d/%d (frozen) - using %s view", fold, epoch, cfg.epochs, view_names[view_idx])
    ...
    for epoch in range(cfg.freeze_epochs + 1, stage2_end):
        ...
        view_names = ["hflip", "vflip", "rotate90", "original"]
        view_idx = (epoch - 1) % 3 if epoch <= cfg.augment_epochs else 3
        LOGGER.info("Fold %d Epoch %d/%d (finetune) - using %s view", fold, epoch, cfg.epochs, view_names[view_idx])
        train_loss = train_one_epoch(cfg, train_loader, model, criterion, optimizer, scaler, device)
        LOGGER.info("Fold %d Epoch %d -> train_loss %.4f", fold, epoch, train_loss)
        val_loss, score, r2_scores = valid_one_epoch(cfg, valid_loader, model, criterion, device)
        step_scheduler(scheduler, val_loss)
    ...
    if cfg.ema_epochs < cfg.epochs:
        for epoch in range(cfg.ema_epochs + 1, cfg.epochs + 1):
            ...
            view_idx = (epoch - 1) % 3 if epoch <= cfg.augment_epochs else 3
            if cfg.use_ema:
                LOGGER.info("Fold %d Epoch %d/%d (ema) - using %s view", fold, epoch, cfg.epochs, view_names[view_idx])
            else:
                LOGGER.info("Fold %d Epoch %d/%d (finetune) - using %s view", fold, epoch, cfg.epochs, view_names[view_idx])
