...
class CFG:
    ...
    augment_epochs: int = 15  # Number of epochs to use augmented views (hflip, vflip, rotate90)
    ...
    output_dir: Path = field(default_factory=lambda: Path(os.environ.get("OUTPUT_DIR", "output/exp-v174")))
...
def get_tta_transforms(img_size: int) -> list[A.Compose]:
    ...
    During training:
    - Epochs 1-15: Use augmented views (hflip, vflip, rotate90) - 4x data
    - Epochs 16-25: Use only original view
    """
    ...
    # Return all 4 views: [hflip, vflip, rotate90, original]
...
def get_train_transforms_for_epoch(img_size: int, epoch: int, augment_epochs: int = 15) -> tuple[A.Compose, int]:
    ...
    Args:
        ...
        augment_epochs: Number of epochs to use augmented views (default: 15)
    
    Returns:
        tuple: (A.Compose transform, view_idx) for the current epoch
    """
    tta_transforms = get_tta_transforms(img_size)
    
    if epoch <= augment_epochs:
        # First N epochs: randomly select from augmented views (hflip, vflip, rotate90)
        # tta_transforms[0:3] are hflip, vflip, rotate90
        view_idx = random.randint(0, 3)  # Randomly select from indices 0, 1, 2
        return tta_transforms[view_idx], view_idx
    else:
        # Remaining epochs: use only original view
        # return tta_transforms[3], 3  # original_view
        view_idx = random.randint(0, 3)  # Randomly select from indices 0, 1, 2
        return tta_transforms[view_idx], view_idx
...
class BiomassDataset(Dataset):
    ...
    TTA-style training: uses the same transform for all three image views (left, right, full)
    within each epoch, but the transform changes across epochs.
    """
    ...
    def set_transform(self, transform: A.Compose):
        """Update the transform for a new epoch."""
        self.transform = transform
    ...
    def __getitem__(self, idx: int):
        ...
        # Apply the same transform to all three views for consistent TTA augmentation
...
class BiomassModel(nn.Module):
    """Shared backbone with regression heads and optional species classification head."""

    def __init__(self, model_name: str, pretrained: bool = True, pretrained_path: str = '', num_species: int = 0):
        super().__init__()
        if pretrained_path:
    ...
    def _make_classification_head(self, num_classes: int) -> nn.Sequential:
        """Create a classification head for species prediction."""
        hidden = self.n_features // 2
...
class PhysicsConsistencyLoss(nn.Module):
    """
    Physics-consistent loss using GaussianNLLLoss with optional species classification.
    ...
    def forward(self, preds, targets, species_targets=None):
        """
        preds: tuple of ((mean, var), ...) for each head, optionally with species_logits at the end
               With species: ((total_mean, total_var), ..., species_logits)
               Without species: ((total_mean, total_var), ...)
        ...
        """
        # Check if species prediction is included (6 elements vs 5)
        if len(preds) == 6:
            ...
            species_logits = preds
        else:
            (pred_total_mean, pred_total_var), \
            (pred_gdm_mean, pred_gdm_var), \
            (pred_green_mean, pred_green_var), \
            (pred_ndvi_mean, pred_ndvi_var), \
            (pred_height_mean, pred_height_var) = preds
            species_logits = None
...
def train_one_epoch(
    ...
    for step, (left, right, full_image, targets, _, species_targets) in pbar:
        left = left.to(device)
        ...
        with autocast(device_type='cuda', enabled=cfg.apex):
            ...
            loss = criterion(preds, targets, species_targets)
...
def valid_one_epoch(
    ...
    try:
        with torch.no_grad():
            ...
            for left, right, full_image, train_targets, eval_targets, species_targets in pbar:
                left = left.to(device)
                right = right.to(device)
                full_image = full_image.to(device)
                ...
                with autocast(device_type='cuda', enabled=cfg.apex):
                    # Model returns tuples of (mean, var) for each head, optionally with species logits
                    preds = model(left, right, full_image)
                    loss = criterion(preds, train_targets, species_targets)

                # Extract mean values for R2 calculation (handle both 5 and 6 element outputs)
                if len(preds) == 6:
                    (pred_total_mean, _), (pred_gdm_mean, _), (pred_green_mean, _), _, _, _ = preds
                else:
                    (pred_total_mean, _), (pred_gdm_mean, _), (pred_green_mean, _), _, _ = preds
...
def run_fold(cfg: CFG, df: pd.DataFrame, fold: int, device: torch.device) -> tuple[float, np.ndarray]:
    ...
    train_dataset = BiomassDataset(
        train_df, initial_train_transform, cfg.image_dir, cfg.target_train, cfg.target_eval
    )
    valid_dataset = BiomassDataset(
        valid_df, get_valid_transforms(cfg.img_size), cfg.image_dir, cfg.target_train, cfg.target_eval
    )
    ...
    model = BiomassModel(cfg.model_name, pretrained=True, pretrained_path=cfg.pretrained_path, num_species=cfg.num_species).to(device)
